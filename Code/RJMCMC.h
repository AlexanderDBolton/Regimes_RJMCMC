...
template< class T>
 ...
 public:
  rj(double = 0, double =1, long long int = 10, int = 1000, long long int =1, long long int=100, bool=0,Particle<T> * = NULL, int seed = 0, bool = false,bool=false);
  rj(int argc, char *argv[], Particle<T> * =NULL, int seed = 0, bool = false);  
  ...
  // Particle<T> * get_MAP() const {return m_MAP;};
  ...
  //virtual int getMAPlength() const = 0;
  ...
  void calculate_sample_histogram( bool mv=false, unsigned int num_bins=50, bool bounded=false, bool weighted_histogram=false, bool calculate_divergence=false, Divergence_Type divergence_type=BIAS, Loss_Function loss_fn=MINIMAX, unsigned int look_up_length=0, bool estaimte_cor=false, unsigned int max_dim=0 );
 ...
 protected:
  ...
  bool m_count_dimension_frequencies;//determines whether or not to use dimension_frequency_count and map_dimension;
  double m_bin_width;
  ...
  bool m_printing_sample_value;//only valid of m_printing_sample == true
  bool m_printing_sample_dimension;//only valid of m_printing_sample == true
  bool m_printing_sample_posterior;//only valid of m_printing_sample == true

};

template<class T>
rj<T>::rj(double begin, double end, long long int its, int max, long long int thin, long long int burnin, bool calcKL,Particle<T> * initial,int se, bool store_sample,bool discrete)
:m_start_time(begin),m_end_time(end),m_iterations(its),m_max_theta(max),m_thinning(thin),m_burnin(burnin),m_initial_sample(initial),m_calculate_div(calcKL),m_seed(se),m_storing_sample(store_sample),m_discrete(discrete),m_start_cps(begin)
  {
    m_start_cps=m_start_time;
...
template<class T>
  rj<T>::~rj()                                                                                
{
  //cout<<"rj destructor called"<<endl;
...
template<class T>
  ...
  m_use_function_criteria = false;
  m_count_dimension_frequencies = true;
  ...
  m_num_bins = 50;
  m_bin_width = (m_end_time-m_start_time)/m_num_bins;
  ...
  m_printing_sample_value = !true;
  m_printing_sample_dimension = true;
  m_printing_sample_posterior = true;
  m_hill_climbing = false;
  m_importance_sampling = false;
  m_current_log_importance_weight = 0;
  m_current_importance_weight = 1;
  m_sum_importance_weights = m_sum_thinned_importance_weights = 0;
  //cout<<"rj constructor called"<<endl;
}

template<class T>
void rj<T>::set_function_criteria(unsigned int grid){

  m_use_function_criteria = true;
  m_length_grid=grid;
  ...
  void  rj<T>::runsimulation()
{
  ...
  while(m_continue_loop){
    ...
    u1=gsl_ran_flat(r,0,1);//(double)rand()/(double)RAND_MAX;
    //birth
    if(u1<=m_b/* || i==(-m_burnin)*/){
      //  cout<< "birth" << endl;
	...
	if(!m_k)
	  ...
	  m_log_proposal_ratio = (m_max_theta == 1) ? -LOG_TWO:LOG_THREE-LOG_TWO;
	...
	gsl_b=1;
	cerr<<"gsl is shit"<<" "<<*new_value<<" "<<m_iters<<endl;
      }
      if(m_iters>0)
	m_birth_attempt++;
    }
    //death
      ...
      //   cout<< "death"<< endl;
      index_theta_delete = static_cast<int>( gsl_ran_flat( r, 0, m_k ) );//zarand() % m_k; 
	...
	m_log_proposal_ratio = (m_max_theta == 1) ? LOG_TWO:LOG_TWO-LOG_THREE;
    ...
    //move
      ...
      //   cout<< "move"<< endl;
      index_theta_move = static_cast<unsigned int>( gsl_ran_flat( r, 0, m_k ) );//rand() % m_k;
    ...
    /*if(m_iters==-m_burnin){
      ...
      m_MAP=copy_particle(m_current_particle);
      }

    if(m_current_particle->get_log_posterior()>m_MAP->get_log_posterior()){
      delete m_MAP;
    ...
    if(m_count_dimension_frequencies && m_iters>0)
    ...
    if(m_calculate_sample_histogram && m_iters>0)
      update_histograms();
...
template<class T>
  void rj<T>::start_printing_sample(){
...
template<class T>
  ...
  if( m_sample_dimensions_stream.is_open() )
    m_sample_dimensions_stream<<m_k<<" ";//current_particle->get_dim_theta()<<" ";
  if( m_sample_logposterior_stream.is_open() )
...
void rj<T>::calculate_sample_histogram( bool mv, unsigned int num_bins, bool bounded, bool weighted_histogram, bool calculate_divergence, Divergence_Type divergence_type, Loss_Function loss_fn, unsigned int look_up_length, bool estimate_cor, unsigned int max_dim ){ 
  ...
  m_bin_width = (m_end_time-m_start_cps)/m_num_bins;
  
  if( !m_histogram ){
    if(!calculate_divergence){
      m_histogram = new Histogram_Type<T>( m_start_cps, m_end_time, m_num_bins, m_bin_width, bounded, weighted_histogram, m_calculate_1d_sample_histogram, max_dim );
    }
    else{
      m_histogram = new Histogram_Type<T>( m_start_cps, m_end_time, m_num_bins, m_bin_width, bounded, weighted_histogram, m_calculate_1d_sample_histogram, max_dim, NULL, true, divergence_type, loss_fn, look_up_length, estimate_cor );
...
template<class T>
  ...
  if(!m_mean_function_of_interest)
    return;
  unsigned int iterations = m_iters/m_thinning;
...
template<class T>
  void rj<T>::update_dimension_distribution(){
    unsigned int current_dimension = m_k;//m_current_particle->get_dim_theta();
    if(!m_importance_sampling){

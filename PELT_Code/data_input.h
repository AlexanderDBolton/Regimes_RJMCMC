...
public:
    data_input(string data_filename, string data_times_filename = "uniform_arrivals", unsigned long int end_time = 1, string model_type = "gaussian", unsigned long int number_of_states = 3, unsigned long int diff = 1);
    ~data_input();
    void print_data();
...
data_input::data_input(string data_filename, string data_times_filename, unsigned long int end_time, string model_type, unsigned long int number_of_states, unsigned long int diff):m_diff(diff), m_number_of_states(number_of_states) {
    if(model_type == "gaussian") {
        ...
        double cumulative_sum = 0, number_of_observations = 0, cumulative_sum_of_log_factorials = 0, data_value;
        if (data_times_filename == "uniform_arrivals") {
            while (dataFile >> data_value) {
                cumulative_sum += data_value;
                number_of_observations++;
                cumulative_sum_of_log_factorials += lgamma(data_value + 1);
                if (actual_obs % m_diff == m_diff - 1) {
                    vector<double> temp_vec(3, 0);
                    temp_vec[0] = cumulative_sum;
                    temp_vec[1] = number_of_observations;
                    temp_vec[2] = cumulative_sum_of_log_factorials;
                    m_cumulative_data[record_index] = temp_vec;
                    cumulative_sum = 0;
                    number_of_observations = 0;
                    cumulative_sum_of_log_factorials = 0;
                    record_index++;
                }
                actual_obs++;
            }
        ...
        ifstream dataFile(data_filename.c_str(), ifstream::in);
        unsigned long int actual_obs = 0, record_index = 0; //actual_obs keeps track of the actual observation that we are feeding in, obs_index keeps track of where this observation is stored in m_cumulative_observations
        long int previous_data_value, data_value;
        ...
        if (data_times_filename == "uniform_arrivals") {
            ...
            while (dataFile >> data_value) {
                if (-0.5 < data_value && -0.5 < previous_data_value) { // sometimes -1 is used to represent a missing value, don't include missing values
            ...
            unsigned long int time_value, max_time = m_diff;
            dataFile >> previous_data_value;
            timesFile >> time_value;
            while (timesFile >> time_value && dataFile >> data_value) {
                if (time_value >= max_time) {
                    m_cumulative_data[record_index] = transitions_vector;
                    fill(transitions_vector.begin(), transitions_vector.end(), 0);
                    while (time_value >= max_time){
    ...
    cout << "number of rows: " << m_cumulative_data.size() << endl;
    //print_data();
